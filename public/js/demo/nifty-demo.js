...
$(document).ready(function () {
    ...
    var demoSetBody         = $('#demo-set'),
        ...
        $("#mainnav-container a").each(function () 
            {
                let url = window.location.href;
                let link = this.href;

                let url_arreglo = url.split('/');
                let link_arreglo = link.split('/');

                let url_activo = "";
                let link_activo = "";

                let prefijo = url_arreglo[0].toLowerCase();

                if(prefijo == "http:" || prefijo == "https:")
                {
                    url_activo = url_arreglo[0] + '//' + url_arreglo[2] + '/' + url_arreglo[3];
                    link_activo = link_arreglo[0] + '//' + link_arreglo[2] + '/' + link_arreglo[3];
                }else{
                    url_activo = url_arreglo[0] + '/' + url_arreglo[2];
                    link_activo = link_arreglo[0] + '/' + link_arreglo[2];
                }
                
                 if (link_activo == url_activo)
                 {
                //    $(this).parent().parent().prev().addClass("sub-menu-txt");
                //    $(this).parent().parent().parent().addClass("sub-menu-txt");
